...
const int BREAK_THRESHOLD = 1000000;
...
void pinHandler1(void) {
	...
	if (digitalRead(BEAM1) == 1) {
		...
		if (diffTimeMicro(temp, beam1RiseLatest) >= BREAK_THRESHOLD) {
			beam1RiseOld = beam1RiseLatest;
			setTime(&beam1RiseLatest);
			printf("--Beam 1 Rise\n");
		} else{
			printf("--Fuzz avoided on beam 1 rise\n");
		}
	}
}
...
void pinHandler2(void) {
	...
	if (digitalRead(BEAM2) == 1) {
		//if current read is a 1, then fall
		if (diffTimeMicro(temp, beam2FallLatest) >= BREAK_THRESHOLD) {
		...
		if (diffTimeMicro(temp, beam2RiseLatest) >= BREAK_THRESHOLD) {
			...
			printf("--Fuzz avoided on beam 2 rise\n");
		}
	}
}
